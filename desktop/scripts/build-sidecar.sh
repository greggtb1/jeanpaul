...
SPEC="$DESKTOP/scripts/jeanpaul-engine.spec"
...
OUT="$DESKTOP/scripts/dist/jeanpaul-engine"
if [[ "$OS" == "Darwin" ]]; then
  if [[ "$ARCH" == "arm64" ]]; then
    TARGET="$BIN_DIR/jeanpaul-engine-aarch64-apple-darwin"
  else
    TARGET="$BIN_DIR/jeanpaul-engine-x86_64-apple-darwin"
  fi
elif [[ "$OS" == MINGW* ]] || [[ "$OS" == MSYS* ]] || [[ "$OS" == CYGWIN* ]] || [[ "${OS:-}" == "Windows_NT" ]]; then
  OUT="$DESKTOP/scripts/dist/jeanpaul-engine.exe"
  TARGET="$BIN_DIR/jeanpaul-engine-x86_64-pc-windows-msvc.exe"
else
  TARGET="$BIN_DIR/jeanpaul-engine"
fi
